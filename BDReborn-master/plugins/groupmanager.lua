...
---------------Security Mode---------------
local function security_mode(msg, data, target) 
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not is_mod(msg) then
if not lang then
 return "_>>_ `You're Not` *Moderator*"
else
 return "_>>_ `شما` *مدیر* `نمیباشید`"
end
end

local security_mode = data[tostring(target)]["settings"]["security_mode"] 
if security_mode == "kick" then
if not lang then
 return "_>>_ *Security* `mode Is Already On` *Kick Mode*"
elseif lang then
 return "_>>_ `حالت` *امنیتی* `از قبل روی` *اخراج* `بود`"
end
else
 data[tostring(target)]["settings"]["security_mode"] = "kick"
save_data(_config.moderation.data, data) 
if not lang then
 return "_>>_ *Security* `mode Changed To` *Kick mode*"
else
 return "_>>_ `حالت` *امنیتی* `تغییر یافت به ` *اخراج*"
end
end
end
...
local security_mode = data[tostring(target)]["settings"]["security_mode"] 
if security_mode == "del" then
if not lang then
 return "_>>_ *Security* `mode Is Already On` *Del Mode*"
elseif lang then
 return "_>>_ `حالت` *امنیتی* `از قبل روی` *حذف* `بود`"
end
else
 data[tostring(target)]["settings"]["security_mode"] = "del"
save_data(_config.moderation.data, data) 
if not lang then
 return "_>>_ *Security* `mode Changed To` *Del mode*"
else
 return "_>>_ `حالت` *امنیتی* `تغییر یافت به`  *حذف*"
end
end
end
...
local security_mode = data[tostring(target)]["settings"]["security_mode"] 
if security_mode == "disable" then
if not lang then
 return "_>>_ *Security* `mode Is Already On` *Disable*"
elseif lang then
 return "_>>_ `حالت` *امنیتی* `از قبل روی` *غیر فعال* `بود`"
end
else
 data[tostring(target)]["settings"]["security_mode"] = "disable"
save_data(_config.moderation.data, data) 
if not lang then
 return "_>>_ *Security* `mode Changed To` *Disable mode*"
else
 return "_>>_ `حالت` *امنیتی* `تغییر یافت به`  *غیر فعال*"
end
end
end
---------------Lock spam-------------------
...
if data[tostring(target)]["settings"] then		
if not data[tostring(target)]["settings"]["security_mode"] then			
data[tostring(target)]["settings"]["security_mode"] = "kick"		
end
end
...
if matches[1] == "security" and is_mod(msg) then
local target = msg.to.id
if matches[2] == "disable" then
return security_mode(msg, data, target)
end
